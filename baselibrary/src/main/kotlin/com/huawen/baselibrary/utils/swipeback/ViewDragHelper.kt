...
private constructor(context: Context, private val mParentView: ViewGroup?, private val mCallback: Callback?) {
    ...
    private fun computeSettleDuration(child: View, dx: Int, dy: Int, xvel: Int, yvel: Int): Int {
        ...
        val xduration = computeAxisDuration(dx, xvel, mCallback?.getViewHorizontalDragRange(child) ?: 0)
        val yduration = computeAxisDuration(dy, yvel, mCallback?.getViewVerticalDragRange(child) ?: 0)

        return (xduration * xweight + yduration * yweight).toInt()
    }

    private fun computeAxisDuration(delta: Int, velocity_: Int, motionRange: Int): Int {
        var velocity = velocity_
        if (delta == 0) {
            return 0
        }
        val width = mParentView?.width ?: 0
     ...
     * method from [android.view.View.computeScroll] or
    ...
    internal fun tryCaptureViewForDrag(toCapture: View?, pointerId: Int): Boolean {
        ...
        if (toCapture != null && mCallback?.tryCaptureView(toCapture, pointerId) == true) {
    ...
    protected fun canScroll(v: View, checkV: Boolean, dx: Int, dy: Int, x: Int, y: Int): Boolean {
        ...
        return checkV && (ViewCompat.canScrollHorizontally(v, -dx) || ViewCompat.canScrollVertically(
            v,
            -dy
        ))
    }
    ...
    private fun checkNewEdgeDrag(delta: Float, odelta: Float, pointerId: Int, edge: Int): Boolean {
        ...
        if (absDelta < absODelta * 0.5f && mCallback?.onEdgeLock(edge) == true) {
    ...
    private fun checkTouchSlop(child: View?, dx: Float, dy: Float): Boolean {
        ...
        val checkHorizontal = mCallback?.getViewHorizontalDragRange(child) ?: 0 > 0
        val checkVertical = mCallback?.getViewVerticalDragRange(child) ?: 0 > 0
    ...
    private fun dragTo(left: Int, top: Int, dx: Int, dy: Int) {
        ...
        if (dx != 0) {
            clampedX = mCallback?.clampViewPositionHorizontal(capturedView!!, left, dx) ?: 0
            capturedView!!.offsetLeftAndRight(clampedX - oldLeft)
        }
        if (dy != 0) {
            clampedY = mCallback?.clampViewPositionVertical(capturedView!!, top, dy) ?: 0
    ...
    fun findTopChildUnder(x: Int, y: Int): View? {
        val childCount = mParentView?.getChildCount() ?: 0
        for (i in childCount - 1 downTo 0) {
            val idx = mCallback?.getOrderedChildIndex(i) ?: -1
            val child = if (idx != -1) {
                mParentView?.getChildAt(idx)
            } else {
                null
            }
            if (x >= child?.left ?: 0 && x < child?.right ?: 0 && y >= child?.top ?: 0
                && y < child?.bottom ?: 0
            ) {
                return child
            }
    ...
    private fun getEdgeTouched(x: Int, y: Int): Int {
        ...
        if (x < mParentView?.left ?: 0 + edgeSize)
            result = EDGE_LEFT
        if (y < mParentView?.top ?: 0 + edgeSize)
            result = EDGE_TOP
        if (x > mParentView?.right ?: 0 - edgeSize)
            result = EDGE_RIGHT
        if (y > mParentView?.bottom ?: 0 - edgeSize)
            result = EDGE_BOTTOM
