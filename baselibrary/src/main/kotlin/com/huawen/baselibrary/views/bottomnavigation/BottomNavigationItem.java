...
public class BottomNavigationItem {
    ...
    private boolean textSizeAvailable = false;
    ...
    public BottomNavigationItem setActiveTextSize(int size) {
        this.mActiveTextSize = size;
        textSizeAvailable = true;
        return this;
    }

    public BottomNavigationItem setInActiveTextSize(int size) {
        this.mInActiveTextSize = size;
        textSizeAvailable = true;
        return this;
    }


    public int getTextSize(boolean inActive) {
        if (textSizeAvailable) {
            if (inActive) {
                return mInActiveTextSize;
            } else {
                return mActiveTextSize;
            }
        } else {
            return -1;
        }
    ...
    boolean isInActiveTextSizeAvailable() {
        return textSizeAvailable;
    }
    ...
    public int getBadgeCount(){
        if (mBadgeItem!=null&&mBadgeItem instanceof TextBadgeItem){
            try {
                int count=Integer.parseInt(((TextBadgeItem) mBadgeItem).getTextView().get().getText().toString());
                return count;
            }catch (Exception e){
            }
        }
    ...
    public void setBadgeCount(int count) {
        if (mBadgeItem != null) {
            if (count > 0) {
                if (mBadgeItem instanceof TextBadgeItem) {
                    ((TextBadgeItem) mBadgeItem).setText("" + count);
                } else if (mBadgeItem instanceof ShapeBadgeItem) {
                }
                mBadgeItem
                        .show(true);
            } else {
                if (mBadgeItem instanceof TextBadgeItem) {
                    ((TextBadgeItem) mBadgeItem).setText("");
                } else if (mBadgeItem instanceof ShapeBadgeItem) {
                }
                mBadgeItem
                        .hide(true);
