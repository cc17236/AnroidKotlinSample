...
import com.huawen.baselibrary.views.banners.listener.OnBannerClickListener
import com.huawen.baselibrary.views.banners.listener.OnBannerListener
import com.huawen.baselibrary.views.banners.loader.ImageLoaderInterface
import com.huawen.baselibrary.views.banners.view.BannerViewPager

class Banner : FrameLayout, androidx.viewpager.widget.ViewPager.OnPageChangeListener {
    ...
    private var mLayoutResId = R.layout.banner
    ...
    private var imageViews: MutableList<View>? = null
    ...
    private var imageLoader: ImageLoaderInterface<out View>? = null
    ...
    private var bannerListener: OnBannerClickListener? = null
    ...
    private fun initView(context: Context, attrs: AttributeSet?) {
        ...
        bannerDefaultImage = view.findViewById<View>(R.id.bannerDefaultImage) as ImageView
        viewPager = view.findViewById<View>(R.id.bannerViewPager) as BannerViewPager
        titleView = view.findViewById<View>(R.id.titleView) as LinearLayout
        indicator = view.findViewById<View>(R.id.circleIndicator) as LinearLayout
        indicatorInside = view.findViewById<View>(R.id.indicatorInside) as LinearLayout
        bannerTitle = view.findViewById<View>(R.id.bannerTitle) as TextView
        numIndicator = view.findViewById<View>(R.id.numIndicator) as TextView
        numIndicatorInside = view.findViewById<View>(R.id.numIndicatorInside) as TextView
    ...
    private fun initViewPagerScroll() {
        try {
            val mField = androidx.viewpager.widget.ViewPager::class.java.getDeclaredField("mScroller")
    ...
    fun setImageLoader(imageLoader: ImageLoaderInterface<out View>): Banner {
    ...
    fun setBannerAnimation(transformer: Class<out androidx.viewpager.widget.ViewPager.PageTransformer>): Banner {
        try {
     ...
     * @return Banner
     */
    fun setPageTransformer(reverseDrawingOrder: Boolean, transformer: androidx.viewpager.widget.ViewPager.PageTransformer): Banner {
    ...
    fun setImages(imageUrls: List<out Any>?): Banner {
        if (this.imageUrls != null && imageUrls != null) {
            update(imageUrls as MutableList<Any>)
        } else {
            this.imageUrls = imageUrls as? ArrayList<Any>
            this.count = imageUrls?.size ?: 0
        }

        return this
    }

    fun update(imageUrls: MutableList<Any>, titles: MutableList<String>) {
        this.titles!!.clear()
    ...
    fun update(imageUrls: MutableList<out Any>) {
        this.imageUrls?.clear()
        this.imageViews?.clear()
        this.indicatorImages?.clear()
        this.imageUrls?.addAll(imageUrls as Collection<Any>)
        this.count = this.imageUrls?.size ?: 0
        start()
    }
...
//    fun setIndicatorVisable(visable:Boolean){
//        indicator!!.visibility= if(visable) View.VISIBLE else View.GONE
//        indicator!!.visibility = if(visable) View.VISIBLE else View.GONE
//        numIndicator!!.visibility = if(visable) View.VISIBLE else View.GONE
//        numIndicatorInside!!.visibility = if(visable) View.VISIBLE else View.GONE
//        indicatorInside!!.visibility = if(visable) View.VISIBLE else View.GONE
//        bannerTitle!!.visibility = if(visable) View.VISIBLE else View.GONE
//        titleView!!.visibility = if(visable) View.VISIBLE else View.GONE
//    }
    fun start(): Banner {
    ...
    private fun setImageList(imagesUrl: List<*>?) {
        ...
        for (i in 0..count + 1) {
            ...
            if (imageLoader != null)
                (imageLoader as? ImageLoaderInterface<View>)?.displayImage(context, url, imageView)
            else
    ...
    private fun setData() {
        ...
        setOffscreenPageLimit(imageViews?.size ?: 1)
    ...
    fun getItem(position: Int): Any? {
        return if (imageUrls != null && imageUrls!!.size > position) imageUrls!![position] else null
    }

    internal inner class BannerPagerAdapter : androidx.viewpager.widget.PagerAdapter() {
        ...
        override fun instantiateItem(container: ViewGroup, position: Int): Any {
            val v = if (imageViews != null && imageViews!!.size > position) imageViews!![position] else throw RuntimeException("错误呀")
            container.addView(v)
            val view = v
...
//                (viewPager?.get(position) as? ImageView)?.invalidate()
//            }
        }
        if (bannerStyle == BannerConfig.CIRCLE_INDICATOR ||
                bannerStyle == BannerConfig.CIRCLE_INDICATOR_TITLE ||
                bannerStyle == BannerConfig.CIRCLE_INDICATOR_TITLE_INSIDE) {
            val lastPos = (lastPosition - 1 + count) % count
            val pos = (position - 1 + count) % count
            if (indicatorImages != null) {
                if (indicatorImages!!.size > lastPos)
                    indicatorImages!![lastPos].setImageResource(mIndicatorUnselectedResId)
                if (indicatorImages!!.size > pos)
                    indicatorImages!![pos].setImageResource(mIndicatorSelectedResId)
            }
    ...
    @Deprecated("")
    fun setOnBannerClickListener(listener: OnBannerClickListener): Banner {
    ...
    fun setOnPageChangeListener(onPageChangeListener: androidx.viewpager.widget.ViewPager.OnPageChangeListener) {
