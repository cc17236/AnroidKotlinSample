package com.huawen.baselibrary.views.bottomnavigation;
...
import com.huawen.baselibrary.views.bottomnavigation.behaviour.BottomNavBarFabBehaviour;
import com.huawen.baselibrary.views.bottomnavigation.behaviour.BottomVerticalScrollBehavior;
import com.huawen.baselibrary.views.bottomnavigation.utils.Utils;
...
public class BottomNavigationBar extends FrameLayout {
    ...
    public interface BeforeWatcher {
        void ensure(int oldPosition, int position);
    }

    private BeforeWatcher watcher;

    @NotNull
    public BottomNavigationBar setBeforeWatcher(@NotNull BeforeWatcher watcher) {
        this.watcher = watcher;
        return this;
    }

    private void selectTabBefore(int position, int oldPosition) {
        if (watcher != null) {
            watcher.ensure(oldPosition, position);
        }
    }
...
//    }
    ...
    public BottomNavigationBar setNavigationHeight(int height) {
        if (mContainer != null) {
    ...
    public BottomNavigationBar addItems(List<BottomNavigationItem> items) {
        mBottomNavigationItems.addAll(items);
        return this;
    }
     ...
     */
    public BottomNavigationBar initialise() {
    ...
    private void setUpTab(boolean isNoTitleMode, BottomNavigationTab bottomNavigationTab, BottomNavigationItem currentItem, int itemWidth, int itemActiveWidth) {
        ...
        bottomNavigationTab.setOnClickListener(new OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                selectTabBefore(bottomNavigationTabView.getPosition(), mSelectedPosition);
    ...
    private void selectTabInternal(int newPosition, boolean firstTab, boolean callListener, boolean forcedSelection) {
        ...
        if (mSelectedPosition != newPosition) {
            if (callListener && !forcedSelection) {
                rlt = mTabSelectedListener.onTabPreSelected(oldPosition, newPosition);
            }
            if (mBackgroundStyle == BACKGROUND_STYLE_STATIC) {
...
//                            }
        ...
        if (callListener) {
            sendListenerCall(oldPosition, newPosition, rlt, forcedSelection);
        }
    }
     ...
     * @param forcedSelection if bottom navigation bar forced to select tab (in this case call on selected irrespective of previous state
     */
    private void sendListenerCall(int oldPosition, int newPosition, boolean preSelected, boolean forcedSelection) {
...
//                && oldPosition != -1) {
    ...
    public interface OnTabSelectedListener {
        ...
        boolean onTabPreSelected(int oldPosition, int newPosition);
