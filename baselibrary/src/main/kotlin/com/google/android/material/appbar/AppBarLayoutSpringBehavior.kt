...
open class AppBarLayoutSpringBehavior : AppBarLayout.Behavior {
    ...
    private fun checkShouldSpringRecover(coordinatorLayout: androidx.coordinatorlayout.widget.CoordinatorLayout?, abl: AppBarLayout) {
        if (offsetSpring > 0) animateRecoverBySpring(coordinatorLayout, abl)
    }

    private fun animateFlingSpring(coordinatorLayout: androidx.coordinatorlayout.widget.CoordinatorLayout?, abl: AppBarLayout, originNew: Int) {
        if (mFlingAnimator == null) {
    ...
    private fun animateRecoverBySpring(coordinatorLayout: androidx.coordinatorlayout.widget.CoordinatorLayout?, abl: AppBarLayout) {
    ...
    override fun onMeasureChild(parent: androidx.coordinatorlayout.widget.CoordinatorLayout, child: AppBarLayout, parentWidthMeasureSpec: Int, widthUsed: Int, parentHeightMeasureSpec: Int, heightUsed: Int): Boolean {
    ...
    internal fun getHeaderExpandedHeight(appBarLayout: AppBarLayout): Int {
        var range = 0
        var i = 0
    ...
    internal override fun onFlingFinished(parent: androidx.coordinatorlayout.widget.CoordinatorLayout?, layout: AppBarLayout) {
        snapToChildIfNeeded(parent, layout)
        animateRecoverBySpring(parent, layout)
    }

    private fun snapToChildIfNeeded(coordinatorLayout: androidx.coordinatorlayout.widget.CoordinatorLayout?, abl: AppBarLayout) {
            ...
            if (flags and AppBarLayout.LayoutParams.FLAG_SNAP == AppBarLayout.LayoutParams.FLAG_SNAP) {
                ...
                if (checkFlag(
                    ...
                    snapBottom += ViewCompat.getMinimumHeight(offsetChild)
                        ...
                        flags,
                        AppBarLayout.LayoutParams.FLAG_QUICK_RETURN or AppBarLayout.LayoutParams.SCROLL_FLAG_ENTER_ALWAYS
                    )
                ) {
                ...
                animateOffsetTo(coordinatorLayout, abl,
                        MathUtils.clamp(newOffset, -abl.totalScrollRange, 0), 0f)
            }
        }
    }
    ...
    internal override fun setHeaderTopBottomOffset(coordinatorLayout: androidx.coordinatorlayout.widget.CoordinatorLayout?,
    ...
    internal fun setHeaderTopBottomOffset(coordinatorLayout: androidx.coordinatorlayout.widget.CoordinatorLayout?,
                                          appBarLayout: AppBarLayout?, newOffset_: Int, minOffset: Int, maxOffset: Int, type: Int): Int {
        var newOffset = newOffset_
...
//                appBarLayout.dispatchOffsetUpdates(topAndBottomOffset)
    ...
    private fun updateSpringByScroll(coordinatorLayout: androidx.coordinatorlayout.widget.CoordinatorLayout?, appBarLayout: AppBarLayout, type: Int, originNew: Int): Int {
        val consumed: Int
    ...
    private fun shouldJumpElevationState(parent: androidx.coordinatorlayout.widget.CoordinatorLayout, layout: AppBarLayout): Boolean {
        ...
        while (i < size) {
            ...
            val lp = dependency.layoutParams as androidx.coordinatorlayout.widget.CoordinatorLayout.LayoutParams
            val behavior = lp.behavior

            if (behavior is AppBarLayout.ScrollingViewBehavior) {
                val behaviorImp = behavior as AppBarLayout.ScrollingViewBehavior?
                return behaviorImp?.overlayTop != 0
            }
            i++
        }
        return false
    ...
    private fun updateSpringOffsetByscroll(coordinatorLayout: androidx.coordinatorlayout.widget.CoordinatorLayout?, appBarLayout: AppBarLayout?, offset: Int) {
    ...
    private fun updateSpringHeaderHeight(coordinatorLayout: androidx.coordinatorlayout.widget.CoordinatorLayout?, appBarLayout: AppBarLayout, offset: Int) {
        ...
        val layoutParams = appBarLayout.layoutParams as androidx.coordinatorlayout.widget.CoordinatorLayout.LayoutParams
    ...
    private fun updateAppBarLayoutDrawableState(parent: androidx.coordinatorlayout.widget.CoordinatorLayout,
                                                layout: AppBarLayout, offset: Int, direction: Int,
                                                forceJump: Boolean) {
        val child = getAppBarChildOnOffset(
            layout,
            offset
        )
        if (child != null) {
           ...
           layout.setExpanded(!collapsed)
//           layout.setCollapsedState(collapsed)
            if (Build.VERSION.SDK_INT >= 11 && (forceJump || shouldJumpElevationState(parent, layout))) {
