 ...
 */
class FixableCardView @JvmOverloads constructor(context: Context, attrs: AttributeSet? = null, defStyleAttr: Int = R.attr.cardViewStyle) : FrameLayout(context, attrs, defStyleAttr) {
    ...
    private var memory: Drawable? = null
    ...
    private val mCardViewDelegate = object : CardViewDelegate {
        ...
        override fun getUseCompatPadding(): Boolean {
            return this@FixableCardView.useCompatPadding
        }

        override fun getPreventCornerOverlap(): Boolean {
            return this@FixableCardView.preventCornerOverlap
        }

        override fun setShadowPadding(left: Int, top: Int, right: Int, bottom: Int) {
            mShadowBounds.set(left, top, right, bottom)
            super@FixableCardView.setPadding(left + mContentPadding.left, top + mContentPadding.top,
                    right + mContentPadding.right, bottom + mContentPadding.bottom)
        }

        override fun setMinWidthHeightInternal(width: Int, height: Int) {
            if (width > mUserSetMinWidth) {
                super@FixableCardView.setMinimumWidth(width)
            }
            if (height > mUserSetMinHeight) {
                super@FixableCardView.setMinimumHeight(height)
            }
        }
        ...
        override fun getCardView(): View {
            return this@FixableCardView
        }
    }

    fun shadowColors(start: Int, end: Int) {
        startColor = start
        endColor = end
        if (IMPL is FixableCardViewApi17Impl && Build.VERSION.SDK_INT >= 17) {
            IMPL.changeShadowColor(mCardViewDelegate, startColor, endColor)
        }
    }

    init {

        val self = context.obtainStyledAttributes(attrs, FixedRFile.CARD_STYLE, defStyleAttr, R.style.CardView)
        val start = self.getColor(FixedRFile.CARD_STYLE_START, 0)
        val end = self.getColor(FixedRFile.CARD_STYLE_END, 0)
        self.recycle()
    ...
    fun setForceDrawable(forceDrawable: LayerDrawable?) {
        if (forceDrawable != null && mCardViewDelegate != null) {
            if (memory == null) {
                memory = mCardViewDelegate.cardBackground
            }
            background = forceDrawable
        } else if (forceDrawable == null && memory != null) {
            background = memory
            if (IMPL is FixableCardViewApi17Impl && Build.VERSION.SDK_INT >= 17)
                IMPL.invalidate(mCardViewDelegate)
            memory = null
        }
    }
